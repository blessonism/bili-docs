...
p=Path('/tmp/bili-build.log')
if p.exists():
    t='\n'.join(p.read_text(errors='ignore').splitlines()[-5:])
    print(t[:300])
PY2
)
...
systemctl stop bili-docs-frontend 2>/dev/null || systemctl stop bili-docs 2>/dev/null || pkill -f 'next-server.*bili' 2>/dev/null || true
sleep 1
rm -f .next/lock
...
systemctl start bili-docs-frontend
sleep 4
